...
#ADHD group
############################################
# Functinal representation with B-spline #
############################################
...
for(i in 1:n1){
  for(j in 1:p1){
    fd_obj <- smooth.basis(t_seq, adhd[i,j,], basis)$fd
...
for(j in 1:p1){
  ...
  lambda_stronger <- fit$lambda.1se * 2.425
...
adj_est_symm_adhd <- (adj_est & t(adj_est)) * 1  # AND
...
num_edges_s <- sum(adj_est_symm_adhd) / 2
cat("n. of edges:", num_edges_s, "\n")


#Zhao graphs - 2% sparsity
load("G.spa.ctrl.ADHD.RData")
graph.plotter(G.ADHD.sym, coordinate.list, name.tag.vec, "yz", "My Graph")

num_edges_s <- sum(G.ADHD.sym) / 2
cat("n. edges zhao graph:", num_edges_s, "\n")

load("G.spa.ctrl.ADHD.FGlasso.RData")
diag(G.ADHD.sym) <- 0
graph.plotter(G.ADHD.sym, coordinate.list, name.tag.vec, "yz", "My Graph")

num_edges_s <- sum(G.ADHD.sym) / 2
cat("n. edges qiao graph", num_edges_s, "\n")

#Dataset CONTROL

############################################
# Functional representation with b splines #
############################################
#L=6, lambda=lambda.1se, lasso
...
for(j in 1:p1){
  ...
  lambda_stronger <- fit$lambda.1se * 2.42
  coef_list <- coef(fit, s = lambda_stronger)
...
load("G.spa.ctrl.control.FGlasso.RData")
diag(G.control.sym) <- 0
graph.plotter(G.control.sym, coordinate.list, name.tag.vec, "yz", "My Graph")

num_edges_s <- sum(G.control.sym) / 2
cat("n. edges qiao", num_edges_s, "\n")
...
load("G.spa.ctrl.ADHD.RData")
graph.plotter(adj.mat         = G.ADHD.sym, 
              coordinate.list = coordinate.list,
              name.tag.vec    = codes,
              node_color_dict = node_color_dict)
...
plot_difference_graph(adj.mat, G.ADHD.sym, coordinate.list, codes, node_color_dict)
